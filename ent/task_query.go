...
func (tq *TaskQuery) QueryImplant() *ImplantQuery {
...
func (tq *TaskQuery) Clone() *TaskQuery {
	...
	return &TaskQuery{
		...
		withImplant: tq.withImplant.Clone(),
...
func (tq *TaskQuery) WithImplant(opts ...func(*ImplantQuery)) *TaskQuery {
	query := &ImplantQuery{config: tq.config}
	for _, opt := range opts {
		opt(query)
	}
	tq.withImplant = query
	return tq
}
...
func (tq *TaskQuery) sqlAll(ctx context.Context) ([]*Task, error) {
	...
	if query := tq.withImplant; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Task)
		for i := range nodes {
			if nodes[i].task_implant == nil {
				continue
			}
			fk := *nodes[i].task_implant
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "task_implant" returned %v`, n.ID)
			}
			for i := range nodes {
