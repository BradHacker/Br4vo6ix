...
// QueryImplant chains the current query on the "implant" edge.
func (hq *HeartbeatQuery) QueryImplant() *ImplantQuery {
	query := &ImplantQuery{config: hq.config}
...
func (hq *HeartbeatQuery) Clone() *HeartbeatQuery {
	...
	return &HeartbeatQuery{
		...
		withImplant: hq.withImplant.Clone(),
...
// WithImplant tells the query-builder to eager-load the nodes that are connected to
// the "implant" edge. The optional arguments are used to configure the query builder of the edge.
func (hq *HeartbeatQuery) WithImplant(opts ...func(*ImplantQuery)) *HeartbeatQuery {
	query := &ImplantQuery{config: hq.config}
	for _, opt := range opts {
		opt(query)
	}
	hq.withImplant = query
	return hq
}
...
func (hq *HeartbeatQuery) sqlAll(ctx context.Context) ([]*Heartbeat, error) {
	...
	if query := hq.withImplant; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Heartbeat)
		for i := range nodes {
			if nodes[i].heartbeat_implant == nil {
				continue
			}
			fk := *nodes[i].heartbeat_implant
			if _, ok := nodeids[fk]; !ok {
				ids = append(ids, fk)
			}
			nodeids[fk] = append(nodeids[fk], nodes[i])
		}
		query.Where(implant.IDIn(ids...))
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "heartbeat_implant" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Implant = n
			}
		}
	}
